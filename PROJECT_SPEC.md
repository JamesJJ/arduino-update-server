...
3. The version string is sanitized for filename compatibility (non-alphanumeric characters except `.-_` replaced with `-`, truncated to 32 chars). Unless `--no-parse-version` is set, versions starting with `|D:__DATE__|T:__TIME__|` are parsed into `YYYYMMDD-hhmmss` format, with any trailing optional string appended as `-<sanitized-suffix>`.
