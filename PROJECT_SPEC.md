...
A lightweight HTTP server written in Go that serves OTA firmware updates to ESP8266 and ESP32 devices, following the [Arduino ESP8266 HTTP OTA protocol](https://arduino-esp8266.readthedocs.io/en/3.0.0/ota_updates/readme.html#http-server) and the [ESP32 HTTP OTA protocol](https://docs.espressif.com/projects/arduino-esp32/en/latest/ota_web_update.html).
...
| Path | Description |
|------|-------------|
| `/ota` | OTA update endpoint for ESP8266 and ESP32 devices |
...
1. A device sends a request to `/ota` with `x-ESP8266-STA-MAC` (or `x-ESP32-STA-MAC`) and `x-ESP8266-version` (or `x-ESP32-version`) headers.
...
All `x-ESP8266-*` and `x-ESP32-*` request headers, the resolved directory, current version, selected file, and skipped version count are logged to stdout.
...
- The version column is the raw `x-ESP8266-version` or `x-ESP32-version` header value with control characters stripped.
